...
import com.fuevent.api.config.KafkaProperties;
...
class FuEventApiKafkaResourceIT {
    ...
    void setup() {
        ...
        FuEventApiKafkaResource kafkaResource = new FuEventApiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/fu-event-api-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/fu-event-api-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
